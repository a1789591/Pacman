...
class DummyAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    self.get_op_pos = []
    for p in range(len(self.enemy_pos)):
      self.get_op_pos.append(gameState.getAgentPosition(self.enemy_pos[p]))
...
class Agent1(DummyAgent):
  def chooseAction(self, gameState):
    ...
    if self.chasingPacman(gameState):
      closet_pacman_dist = 1000
      pacmans= []
      for op_pos in self.enemy_pos:
        # print(op_pos)
        # dist = self.getMazeDistance(gameState.getAgentState(self.index), gameState.getAgentState(op_pos))
